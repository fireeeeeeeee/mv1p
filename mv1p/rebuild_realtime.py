...
def main(dataset,client):
    ...
    pts1s=np.zeros((3,0))
    preP=None
    for nf in tqdm(range(len(dataset)), desc='reconstruction'):
        ...
        pts0=annots['keypoints2d'][0]
        pts1=annots['keypoints2d'][1]

        mask=(pts0[:,2]>0.6) & (pts1[:,2]>0.6)
        pts0_c=pts0[mask].T
        pts1_c=pts1[mask].T
        pts0_c[2]=1
        pts1_c[2]=1    
        ...
        import cv2
        pts=cv2.triangulatePoints(P0,P1,pts0.T[:2],pts1.T[:2])
        pts/=pts[3]
        pts[3][mask==False]=0
        datas=[]
        data={
...
if __name__=='__main__':
    ...
    if args.body:
        cfg_dataset['keys'].extend(['bbox', 'keypoints'])
    if args.vis2d or args.vis_repro:
        cfg_dataset['keys'].append('img')

    from easymocap.dataset.mvbase import MV1Psocket
    cfg_dataset['hostports'] = args.host2d
    cfg_dataset['nFrames'] = 1000000
    dataset = MV1Psocket(**cfg_dataset)
